...
from iraftk.iraf_stsdas_isophote_ellipse import stsdas_analysis_isophote_ellipse,stsdas_analysis_isophote_bmodel
...
# Initial set up
	...
	def __init_std_catalog_config(self):
		...
		self.std_catalog_colnames = {'apass': self.apass_colnames_dict, 'panstarrs': self.panstarrs_colnames_dict, '2mass':self.twomass_colnames_dict, 'refcat2': self.refcat2_colnames_dict}
...
#Get basic image info
	...
	def __get_image_size_from_fitsheader(self, image_key):
		img_abs = self.images[image_key]
		hdr = fits.open(img_abs)[0].header
		self.photometry_info[image_key]['NX'] = hdr['NAXIS1']
		self.photometry_info[image_key]['NY'] = hdr['NAXIS2']
		...
		for imgkey in self.photometry_info.keys():
			...
			try:
				if not self.photometry_info[imgkey]['rmCR']:
					self.__remove_cosmic_ray(imgkey, which_dir = which_dir, method=method, sigclip=sigclip)
	...
	def source_detection(self,method='daofind', which_dir = None, fistar_ntime_above_bkg = 3, daofind_nsigma_thresh = 5, nmin=5, redo_daofind=True):
		...
		for imgkey in self.photometry_info.keys():
			if which_dir is None:
				if self.photometry_info[imgkey]['bitpix'] == -32:
...
# PSF photometry IRAF
	...
	peakmax=45000, snrmin=50, edge_width = 60, use_tplbase_as_prior = False, tplimgkey=None, xytran_need=True, \
	eyecheck=False, pick_mag_ref_pst = False, reuse_pst=True, overwrite=False, \
	Nmin_warn = 20, Nmax=120, reldiff_cut1 = 0.05,reldiff_cut2=0.01, learning=False, image_dir='raw_image'):
		'''
		...
		if os.path.exists(psffile) and (not overwrite):
			raise ValueError("PSF model for %s already exists. Please set overwrite=1 if you want that.")
		...
		if os.path.exists(pstfile) and reuse_pst:
			...
			if not use_tplbase_as_prior:
				...
				print "Source detection done"

				sround = srs1['SROUND'].data
				dist = np.absolute(sround - np.median(sround))
				distcut = dist[np.argsort(dist)][int(np.floor(len(sround)*0.68))]
	...
	phot_recenter_xy = True, apradius=None, skyin=None, skywidth=None, residual_image = True, subimage=None, \
	residual_smallcut=False, cutout_width=120, update_phot_record=False, which_dir='raw_image'):
		...
		INPUTS:
			...
			phot_recenter_xy: whether to recenter the source during daophot task
		...
		self.__delete_file_if_exist(photfile)
		self.__aperture_photometry_apphot_iraf_single_image(image, coo_list, photfile, options, centering = phot_recenter_xy)
	...
	def psf_photometry_iraf_flt(self, flt, dolist = None, maxsma=None,  renew_psfmodel=False, renew_phot_image=False, \
	renew_phot_target=False, snrmin=50, edge_width=60, tplpsf_prior=False, tplimgkey=None, target_phot_indpt = True, \
	hybrid_model = False, galra=None, galdec=None, galx=None, galy=None, starxs=None, starys=None, peakfitsky='no', \
	allstarfitsky='no', recenter='yes', which_dir='raw_image', debug=0):
		'''
		...
		INPUTS:
			...
			maxsma: maximum semiaxis length for the isophote modelling; only required when hybr == True
			renew_psfmodel:
			...
			galra, galdec: galaxy center in degree; only required when hybrid_model == True
			galx, galy:
			...
			if self.photometry_info[imgkey]['instmag'] == self.photometry_info_init_values['instmag'] or renew_phot_target:
				try:
					if target_phot_indpt:
						if hybrid_model:
							if maxsma is None:
								raise ValueError('maxsma is required for hybrid model')
							if (galra is None or galdec is None) and (galx is None or galy is None):
								raise ValueError('(galra, galdec) or (galx, galy) is required for hybrid model')
							if galra is not None and galdec is not None:
								if starxs is not None:
									print "# WARNING: starlist only allowed when galx, galy are used"
								self.psf_photometry_iraf_single_target_hybrid(imgkey, galra, galdec, maxsma, gal_coord_image=False, niter=3, which_dir=which_dir, fitsky=peakfitsky)
							else:
								#if tplimgkey is None:
								...
								if starxs is not None and starys is not None:
									...
									xstars = None
									ystars = None
								self.psf_photometry_iraf_single_target_hybrid(imgkey, galxc, galyc, maxsma, gal_coord_image=True, xstars=xstars, ystars=ystars, niter=3, which_dir=which_dir, fitsky=peakfitsky)
						else:
	...
	input_image=None, residual_image = False, subimage=None, which_dir='raw_image'):
		'''
		...
		bgvalue = self.photometry_info[imgkey]['bkg']
		stsdas_analysis_isophote_bmodel(isomodel, isoimage, parent_image=input_image, backgr=bgvalue)
	...
	def psf_photometry_iraf_single_target_hybrid(self, imgkey, xgal, ygal, maxsma, fitsky='yes', \
	xstars=None, ystars=None, gal_coord_image=True, niter=1, gal_subimage_smcut=True, \
	isoimage_smcut=True, hybrid_subimage_smcut=True, peak_subimage_smcut=True, which_dir='raw_image'):
		'''
		...
		if not gal_coord_image:
			world_radec = np.array([xgal, ygal]).reshape(1,2)
		...
		spphot = self.iraf_psf_photometry_peak(imgkey, psffile=psffile, peakfile=peakfile, x=xstars, y=ystars, fitrad=None, psfrad=None, fitsky='yes', recenter='no', phot_recenter_xy = False, residual_image = True, subimage=peak_subimage, residual_smallcut=False, update_phot_record=False, which_dir=which_dir)
		print spphot

		self.iraf_get_isophote_model(imgkey,  xgal, ygal, 1, maxsma, input_image=peak_subimage, residual_image= False, which_dir=which_dir)
		isoimage = os.path.join(self.isophote_dir, imgkey_s+'_iso.fits')
		gal_subimage = os.path.join(self.isophote_dir, imgkey_s+'_gal_sub.fits')
		self.__delete_file_if_exist(gal_subimage)
		imarith_iraf(image, '-', isoimage, gal_subimage)

		i = 1
		while i<niter:
			print "iteration #"+str(i)
			peakfile = os.path.join(self.isophote_dir, imgkey_s+'_xys.peak')
			spphot = self.iraf_psf_photometry_peak(imgkey, image=gal_subimage, psffile=psffile, peakfile=peakfile, x=xstars, y=ystars, fitrad=None, psfrad=None, fitsky=fitsky, recenter='yes', phot_recenter_xy = False, residual_image = False, subimage=None, residual_smallcut=True, update_phot_record=False, which_dir=which_dir)
			print spphot
			exfile = ''
		...
		spphot = self.iraf_psf_photometry_peak(imgkey, image=gal_subimage, psffile=psffile, x=xtarget, y=ytarget, fitrad=None, psfrad=None, fitsky=fitsky, recenter='yes', phot_recenter_xy = False, residual_image = True, subimage=subimage, residual_smallcut=False, update_phot_record=True, which_dir=which_dir)
...
#Standard calibration
	...
	def get_calmag_instmag_offset_single_image(self, image_key, photmethod='apphot', aperture_size_fixed=False, renew_existing_photret=True, img_input_which_dir='raw_image', apphot_centering=True, link_stdref_obs_method='surrounding_search', std_catalog = 'apass', panstarrs_photmethod = 'PSF', ):
		...
		if photmethod == 'apphot':
			...
			if os.path.exists(photret_file_abs) and (not renew_existing_photret):
				...
				np.savetxt(photret_file_abs,photret,fmt="%6.2f %6.2f %6.3f %6.3f")
			flt = self.photometry_info[image_key]['flt']
			offset, offset_err = self.stdcal_single_image(image_key,flt, instmag=0, instmagerr=0, link_stdref_obs_method= link_stdref_obs_method, std_catalog = std_catalog, panstarrs_photmethod =  panstarrs_photmethod,  std_world2img_refimage=None,  stds_shift_x=0, stds_shift_y=0, std_obs_matched = False, verbose = 0, update_stdmag=False)
	...
	renew_selfphot_stds = False, secondary_std_photrettable=None, secondard_std_photfile=None):
		...
		if std_obs_matched:
			...
			if std_catalog == 'apass':
				...
				refxymagfile = os.path.join(self.std_ref_dir, 'xymag_'+telcode+imgkey.replace('fits', 'txt'))
				refradecmagfile = os.path.join(self.std_ref_dir, 'radecmag_'+telcode+imgkey.replace('fits', 'txt'))
				bridge_image = os.path.join(self.std_ref_dir, telcode+imgkey)
		...
		print mags_ref_matched
		print mags_input_matched
		cal_mag,cal_magerr, temp_future_use = self.__relative_calibration_single(mags_ref_matched,mags_input_matched, instmag, offset_method=offset_method, sigclipping_first = sigclipping)
	...
	def add_transformed_flt_mags_PanSTARRS(self, refstars, photmethod='PSF'):
		...
		refstars.add_columns([Bmags, Bmagerrs, Vmags, Vmagerrs, Rmags, Rmagerrs, Imags, Imagerrs])

		return refstars

	def add_image_coordinate_PanSTARRS(self, refstars, image, colnames_dict, wcsinfo_hdu='PRIMARY'):
		'''
	...
	def get_expanded_refined_filtered_stds_PanSTARRS(self, refimg, add_transformed_mags = True, add_image_coordinate=True, wcsinfo_hdu='PRIMARY'):
		...
		refstars = self.add_image_coordinate_PanSTARRS(refstars, refimg, colnames_dict, wcsinfo_hdu=wcsinfo_hdu)
	...
	save_std_flt_file = True, save_std_flt_reg = True, wcsinfo_hdu='PRIMARY'):
		...
		refstars = self.add_image_coordinate_PanSTARRS(refstars, reference_image, colnames_dict, wcsinfo_hdu=wcsinfo_hdu)
		#print refstars
...
#matplotlib visualization
	...
	def lc_plot(self, magcolname,flts = 'all',bining =False, bin_width = 0.5,remove_inbin_outliers=False, plot_droped = False,plot_bined_only = False, updatetable=1, extra_filter_table=None, extra_filter_colname=None, extra_filter_colvalue=None):
		'''
		Inputs:
...
#Image transformation
	...
	def __fitsh_fitrans(self, input_image, output_image, trans_file, sx=None, sy=None):
		...
		For details on grmatch refer to http://fitsh.szofi.net/task/fitrans
...
#Common
	...
	def make_mask_image(self, imgkey, mask_image_dir=None, base_image_dir='raw_image', \
...
#Image subtraction
	...
	renew_sub=False, apply_mask=False, which_dir='raw_image'):
		'''
	...
	xc=None, yc=None,  fitsky='yes', recenter='yes', renewphot=False, \
	renew_psfmodel=False, use_tplpst_prior = True,skiplist=None, which_dir='raw_image'):
		...
		for imgkey in self.images.keys():
			...
			fitrad = int(psfrad/4.0*1.3)
			fitsky1no = 'no'
			peakphot = self.iraf_psf_photometry_peak(imgkey, x=xc, y=yc, photfile=photfile, psffile=psffile,peakfile=peakfile, fitrad=fitrad, psfrad=psfrad, fitsky=fitsky1no, recenter=recenter, phot_recenter_xy=True, apradius=apradius, skyin=skyin, skywidth=skywidth, residual_image=True, subimage=subimage, residual_smallcut=True, update_phot_record=False, which_dir='subtracted_image')
			peakmsky = peakphot['MSKY'].data[0]
			peakmag = peakphot['MAG'].data[0]
			peakmagerr = peakphot['MERR'].data[0]
			...
			peakphot = self.iraf_psf_photometry_peak(imgkey, x=xc, y=yc, photfile=photfile, psffile=psffile,peakfile=peakfile, fitrad=fitrad, psfrad=psfrad, fitsky=fitsky2yes, recenter=recenter, phot_recenter_xy=True, apradius=apradius, skyin=skyin, skywidth=skywidth, residual_image=True, subimage=subimage, residual_smallcut=True, update_phot_record=False, which_dir='subtracted_image')
			peakmsky = peakphot['MSKY'].data[0]
			peakmag = peakphot['MAG'].data[0]
			peakmagerr = peakphot['MERR'].data[0]
			...
			fitsky2no = 'no'
			fitrad = int(psfrad/4.0*1.3)
			peakphot = self.iraf_psf_photometry_peak(imgkey, x=xc, y=yc, photfile=photfile, psffile=psffile,peakfile=peakfile, fitrad=fitrad, psfrad=psfrad, fitsky=fitsky2no, recenter=recenter, phot_recenter_xy=True, apradius=apradius, skyin=skyin, skywidth=skywidth, residual_image=True, subimage=subimage, residual_smallcut=True, update_phot_record=False, which_dir='subtracted_image')
			peakmsky = peakphot['MSKY'].data[0]
			peakmag = peakphot['MAG'].data[0]
			peakmagerr = peakphot['MERR'].data[0]
